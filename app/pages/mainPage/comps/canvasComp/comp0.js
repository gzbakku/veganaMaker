...
const init = (pid) => {         //pid referes to the parentPageId, pass this var when you init thiscomp.
  ...
  attach_update_build_for_mobile();
  build();
...
function build(){
  ...
  engine.add.function("draw",()=>{
    if(engine.get.platform() === "electron"){
      ipc.call("update_mobile_view",window.builder);
    }
...
function attach_update_build_for_mobile(){
  if(engine.get.platform("electron")){
    ipc.on("update_build_for_mobile",(e,data)=>{
      if(main){engine.view.remove(main);}
      main = engine.make.div({
        parent:compId,
        class:'comp-canvas-main',
      });
      draw_element(main,data,"",{});
    });
  }
}
